...
def slice_with_neighborhood(data_3d, axis_to_take, idx, neighborhood=0, ratio=1.0):
    ...
    if idx - neighborhood < 0:
        ...
        array_pad = np.zeros((shape[0], shape[1], neighborhood-idx)).astype(np.float32)
        # print(slice_tmp.shape, array_pad.shape)
        slice_to_return = np.concatenate((array_pad, slice_tmp), axis=2)
    elif idx + neighborhood >= axis_len:
        sl[axis_to_take] = slice(idx - neighborhood, axis_len, 1)
        slice_tmp = np.transpose(np.copy(data_3d[tuple(sl)]), transpose[axis_to_take])
        shape = slice_tmp.shape
        array_pad = np.zeros((shape[0], shape[1], idx + neighborhood - axis_len + 1)).astype(np.float32)
        slice_to_return = np.concatenate((slice_tmp, array_pad), axis=2)
    else:
        sl[axis_to_take] = slice(idx - neighborhood, idx + neighborhood + 1, 1)
        slice_to_return = np.transpose(np.copy(data_3d[tuple(sl)]), transpose[axis_to_take])

    if abs(ratio - 1.0) < 1e-3:
        return slice_to_return

    return resize_with_ratio(slice_to_return, ratio)


def resize_with_ratio(image, ratio, modality='t1'):
